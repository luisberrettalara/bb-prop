...
<div class="py-4">
  <div class="row justify-content-center">
    <div class="col-md-5 col-12">
      <form method="POST" action="{{ route('admin.caracteristicas.store') }}" role="form">
      @csrf
        <div class="card">
          <div class="card-header">
            <h4>Nueva Característica</h4>
          </div>
          <div class="card-body">
            <div class="row">
              <div class="col-12">
                <div class="form-group">
                    ...
                    <select id="tipo_caracteristica_id" type="text" class="form-control{{ $errors->has('tipo_caracteristica_id') ? ' is-invalid' : '' }}" name="tipo_caracteristica_id" value="{{ old('tipo_caracteristica_id') }}">
                        ...
                        @foreach ($tipos as $tip)
                            <option value="{{$tip->id}}" {{ old('tipo_caracteristica_id') == $tip->id? 'selected':'' }} >{{$tip->nombre}}</option>
                        @endforeach
                    </select>
                ...
                <div class="form-group">
                    <div class="form-check form-check-inline">
                        <input class="form-check-input{{ $errors->has('es_servicio') ? ' is-invalid' : '' }}" name="es_servicio" type="checkbox" value="1" {{old('es_servicio')?'checked':''}}>
                    </div>
                ...
                <div id="grupo-unidad" class="form-group">
                    <label for="unidad" class="form-label"><strong>Unidad</strong></label>
                    <input type="text" name="unidad" class="form-control{{ $errors->has('unidad') ? ' is-invalid' : '' }}" value="{{ old('unidad') }}">
